...
pub async fn get_byte(name: web::Path<String>, data: web::Json<DataFile>)-> impl Responder {
    let range: (u64, u64) = (data.0.from, data.0.to);
    let status = get_byte_to_server(name.into_inner(), range).await.status();
    HttpResponse::Ok().body(status.as_str().to_string())
}

async fn get_byte_to_server(name: String, range: (u64, u64)) -> Response {
    let mut file = fs::File::open(std::path::Path::new(&format!("./static/{}", name))).unwrap();
    let file_size = file.metadata().unwrap().len();
    let start = range.0;
    let end = range.1;
    ...
    let client = reqwest::Client::new();
    let mut headers = req_header::HeaderMap::new();
    headers.insert(req_header::CONTENT_LENGTH, HeaderValue::from(72));
    let mut url = String::from("http://[::1]:3000/get/");
    url.push_str(&name);
    client
        .post(&url)
